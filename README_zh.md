...
| 0.2 | **给已有数字加历史坐标** —— 过去 60/252 个交易日分位、1日/5日变动、该标的自身区间；覆盖 GEX、IV、期权量 | 最常见的缺口：`Total GEX −6.44B` 单看无法判断 —— 是极端值，还是 SPY 平常的一天？要等本地历史攒够 |
| 0.3 | **波动率期限结构** —— 按到期日的 ATM IV、IV 对已实现波动率、近远月价差、标注财报日 | 没有它，期权那两页只有仓位、没有价格。已实现波动率那条腿需要本地留存每日收盘 |
| 0.4 | **财报与基本面** —— 三表、财报日历、超预期历史 | EDGAR XBRL 与已接好的申报索引不是同一种结构 |
| 0.5 | **可保存的筛选条件 + 装好当天就能用的扫描器** —— 横截面 IV30 分位、IV30/RV20、期权量对自身中位数 | 把一只票放进**全市场**排序，一次扫描就能算；放进**它自己的过去**排序不行 |
| 0.6 | **板块相对的资金流与 IV** | 单票便宜、全市场贵，要先有扫描器的批量通道 |
| 0.7 | **季节性** —— 按月、按年 | 要的是好几年的本地历史，不是一次取数 |
| 0.8 | **预测市场** —— Polymarket 与 Kalshi 作宏观叠加层 | 在别处已跑通，是移植不是发明。**刻意排最后**：它与其余部分连接最弱，也最容易让数据工具滑向信息杂货铺 |
| **1.0** | 完全版 | |

⭐ 这个顺序是听取一位美股期权交易者的评审后改的。他说得很直白：下一步需要的**不是第十一个数据源，
而是让已有的数字可比**。**「这个数字异常吗」问的是数据本身，不是对市场的看法** ——
所以分位、z-score、时间序列变动都稳稳落在「不下结论」之内。
真正挡在外面的是**综合评分**：不做那种把 130 天前的 Form 4 和昨天的期权成交揉成一个数的东西。
